...
- `SessionWorkspace` renders the active Command Code PTY as a native conversation projection by default. Raw `TerminalPane` is mounted only from Advanced session tools or a terminal-required fallback, while the composer remains the normal prompt surface. The projection is display-only and does not replace Command Code runtime truth.
...
Current Phase 9 status on 2026-06-06: complete for the current V1 contract, with mutation/runtime-affecting workbench actions gated.
...
- `src/renderer/src/inspectors/RightInspectorPanel.tsx` owns a compact header switcher for the existing Files, Transcript, Docs, Environment, and IDE inspector modes. It calls only the App-owned `rightInspector` state transitions and does not introduce new inspector transport, file access, hooks/MCP modes, or runtime capability.
- The right-inspector resize handle now exposes the existing App-owned drag-to-collapse path with an accessible label and visible grip. Width persistence still uses the existing `ccgui.right-inspector-width` preference only, and collapse remains transient `rightInspector` state rather than a new persisted setting.
- Right-inspector motion is CSS-only: `.workbench-shell` eases grid column changes, `.right-inspector` uses a short `inspector-slide-in` animation, and `prefers-reduced-motion: reduce` disables both. No runtime state or transport behavior depends on animation timing.
- `src/renderer/src/settings/SettingsReadOnlyCard.tsx` owns shared loading skeleton presentation for read-only Settings discovery sections. It renders only from each section's existing `loading` state and does not introduce new transport, polling, caching, or Settings persistence behavior.
- `docs/reports/WORKBENCH_POLISH_GATE.md` is the current boundary for file actions, IDE actions, git mutations, terminal lifecycle/profile work, editable theme-token controls, and release-fetching changes. Existing read-only status surfaces remain allowed; mutation or runtime-affecting work needs a dedicated gate package.
- `src/renderer/src/workbench/workbenchActions.ts` is a pure preview-only registry for the remaining workbench gate. Settings > Data renders it as a read-only Workbench polish gate card; the registry intentionally has no command strings, callbacks, transport references, or apply actions.
