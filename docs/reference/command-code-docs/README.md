...
- Current GUI status: Settings > Hooks reads only the documented project/user `settings.json` paths through scoped discovery, shows parsed diagnostics, and can preview/apply enable/disable toggles with `.ccgui.bak` backups. Broader command, matcher, timeout, and delete previews have scoped helper/server support, but broader UI/write behavior and execution remain gated.
