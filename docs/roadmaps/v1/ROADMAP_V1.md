...
Status on 2026-06-06: complete and validated for the current V1 contract. Settings > Hooks now has documented user/project config discovery, scoped validated preview/apply writes with backups for enable/disable and command/matcher/timeout/delete edits, read-only hook logs, dry-run payload diagnostics with `execution: not-run`, notification reference alignment, reducer-backed unread/readiness state, and a pure preference-aware notification planner. Real hook command execution, OS notification dispatch, audio playback dispatch, real-session hook payload execution, and runtime-integrated response-ready/input-required events remain gated by `docs/reports/HOOKS_NOTIFICATIONS_GATE.md`.

Initial parser/gate package: `src/core/hooksConfig.ts` now parses documented hook settings from raw `settings.json` text, preserves source scope/path, extracts command hooks, matchers, timeout, enabled state, event order, and blocking capability, rejects invalid JSON/shapes before any future write path, and orders project hooks before user hooks for display. `docs/reports/HOOKS_NOTIFICATIONS_GATE.md` records the write, execution, and notification-readiness boundaries that still block editable hook config and response-ready notifications. This package did not add file reads/writes, server routes, renderer IPC, hook execution, test-payload execution, session readiness, OS notifications, or Command Code settings mutation. Validation receipts: `npm run typecheck`, `npx vitest run` -> `64/64`, and `npm run build`.
...
Status on 2026-06-06: complete and validated for the current V1 contract. Per-session model identity is displayed from session-start or transcript metadata, model picker search/filter is implemented, Settings > Models separates active session selection from Command Code-owned task routing, and documented task-routing previews open the `/configure-models` helper without adding GUI-owned routing config writes. Persistent task routing edits, invented pricing/context metadata, and vision-adapter routing remain gated or future/plugin-owned.
...
Status on 2026-06-06: complete and validated for the current V1 contract. Transcript JSONL parsing, transcript filters, click-to-open artifact chips, right-inspector source labels, read-only resume receipts, recent/live sidebar grouping, recent-context search, per-session mounted terminal panes, safe HTML fallback-to-source previews, readiness labels, multi-session mock independence, browser three-session dogfood, and Electron three-session dogfood are implemented. Editable labels/notes, safe bulk operations, rendered untrusted HTML, file reveal actions from artifact chips, and notification dispatch remain deferred behind named persistence, file-access, untrusted-rendering, and notification gates.

Initial parser package: `src/core/transcriptParser.ts` and `tests/transcript-parser.test.ts` normalize already-provided transcript JSONL text into typed user, assistant, tool, error, event, and unknown timeline entries. The parser preserves raw entries, records invalid JSONL lines as parse errors, ignores blank lines, and does not read, mutate, reveal, or infer state from transcript files. Validation receipts: `npm run typecheck`, `npx vitest run` -> `152/152`, and `npm run build`. Browser, Electron, PTY, headless, and real CLI smoke were not run for this package because no renderer, transport, session lifecycle, file access, command-builder, or runtime behavior changed.
...
Status update 2026-06-06: Sixth polish package added a compact mode switcher to `RightInspectorPanel` for the existing Files, Transcript, Docs, Environment, and IDE inspector modes. The switcher is visible inside the right-inspector header and calls existing App state transitions only; it does not add hooks/MCP inspector modes, file actions, renderer IPC, server routes, transport calls, config writes, file access capability, session lifecycle changes, or Command Code invocation behavior. Validation receipts: `npm run typecheck`, `npm run build`, `npm run smoke:browser`, built route token proof at `http://127.0.0.1:57409/` serving `index-Bn0wWPUV.js` and `index-DFMbXlG2.css`, built asset proof for `right-inspector-switcher`, `InspectorModeButton`, and `right-inspector-switcher-button`, in-app Browser UI receipt clicking Files, Transcript, Docs, Env, and IDE with screenshot `/tmp/ccgui-phase9-inspector-switcher-browser.png`, and built Electron UI receipt clicking the same inspector modes with screenshot `/tmp/ccgui-phase9-inspector-switcher-electron.png`.

Status update 2026-06-06: Seventh polish package made the existing right-inspector resize-to-collapse behavior discoverable with a labeled resize handle and hover/drag grip. Dragging the handle past the existing collapse threshold closes the inspector by clearing the App-owned `rightInspector` state; this does not add new persistence keys, resize routes, renderer IPC, server routes, transport calls, config writes, file access capability, session lifecycle changes, or Command Code invocation behavior. Validation receipts: `npm run typecheck`, `npm run build`, `npm run smoke:browser`, built route token proof at `http://127.0.0.1:57410/` serving `index-BGEnCu0J.js` and `index-BIgUyJ0b.css`, built asset proof for `Resize or collapse inspector` and `right-inspector-resize-handle::before`, in-app Browser drag receipt showing `inspectorCount: 0` with screenshot `/tmp/ccgui-phase9-inspector-collapse-browser.png`, and built Electron drag receipt showing `inspectorCount: 0` with screenshot `/tmp/ccgui-phase9-inspector-collapse-electron.png`.

Status update 2026-06-06: Eighth polish package added restrained right-inspector panel motion: the workbench grid eases column changes and the inspector gets a short slide/fade entry animation with a `prefers-reduced-motion` override. This is CSS-only and does not alter inspector state, persistence, renderer IPC, server routes, transport calls, config writes, file access capability, session lifecycle, or Command Code invocation behavior. Validation receipts: `npm run typecheck`, `npm run build`, `npm run smoke:browser`, built route token proof at `http://127.0.0.1:57411/` serving `index-AB0ie-by.js` and `index-B568GD8W.css`, built asset proof for `inspector-slide-in`, `prefers-reduced-motion`, and `grid-template-columns 140ms ease`, in-app Browser computed-style receipt showing `inspectorCount: 1`, `inspectorAnimationName: inspector-slide-in`, and `shellTransition: grid-template-columns 0.14s` with screenshot `/tmp/ccgui-phase9-inspector-transition-browser.png`, and built Electron computed-style receipt with screenshot `/tmp/ccgui-phase9-inspector-transition-electron.png`.
...
Status update 2026-06-06: Eleventh preview package added a pure `workbenchActions` registry and surfaced it in Settings > Data as a read-only Workbench polish gate card. The card shows implemented read-only file preview status, blocked file/IDE/git/terminal/release actions, and gated preview-only theme token controls with proof requirements from `WORKBENCH_POLISH_GATE.md`; it has no command strings, callbacks, transport calls, apply buttons, renderer IPC, server routes, file access, config writes, session lifecycle changes, CSS token rewrites, or Command Code invocation behavior. Validation receipts: `npm run typecheck`, `npx vitest run tests/workbench-actions.test.ts`, `npm run build`, `npm run smoke:browser`, built route token proof at `http://127.0.0.1:57413/` serving `index-CDcTfQLF.js` and `index-D6JSp2Ur.css`, built asset proof for `Workbench polish gate`, `file-create-rename-delete`, `theme-token-controls`, `settings-workbench-gate-group`, and `WORKBENCH_POLISH_GATE`, in-app Browser Settings > Data receipt showing `found: true`, `blockedCount: 5`, and `previewCount: 1` with screenshot `/tmp/ccgui-phase9-workbench-gate-browser.png`, and built Electron Settings > Data receipt with screenshot `/tmp/ccgui-phase9-workbench-gate-electron.png`.
...
- Make the right inspector behave like a native companion pane for files, docs, transcripts, hooks, MCP, and session artifacts. Implemented so far for the existing files, docs, transcript, environment, and IDE modes with a header switcher; hooks/MCP inspector modes and artifact-specific companion actions remain planned/gated.
- Add restrained page transitions, panel slide-ins, loading skeletons, and resize-to-collapse. Implemented so far for right-inspector grid easing, slide/fade entry motion, discoverable drag-to-collapse, and shared Settings read-only loading skeletons.
...
Status update 2026-06-06: Settings usability package made the Settings content pane the constrained scroll container, added stacked human-readable Settings rows for Hooks config sources, discovered hooks, diagnostics, hook logs, and examples, and upgraded Settings > Agents from preview-only draft copy to preview-confirmed project agent creation through the existing scoped `transport.saveAgent(...)` route. The existing server path guard remains the authority for writes under `<project>/.commandcode/agents/`, and `saveAgent` now creates the validated parent directory for new project agents. This package did not add renderer IPC, broad filesystem capability, Command Code runtime semantics, agent routing behavior, hook execution, OS notifications, session lifecycle changes, or Command Code settings mutation. Validation receipts: `npm run typecheck`, `npx vitest run` -> `173/173`, `npm run build`, `npm run smoke:browser`, built browser route `http://127.0.0.1:57420/` showing Hooks scroll metrics (`scrollHeight 1678`, `clientHeight 1192`, bottom reached, 9 stacked rows) and Settings > Agents created `/tmp/ccgui-settings-agent-nD4rUV/.commandcode/agents/settings-proof-agent.md`; no-project creation disabled with explicit copy after preferences restore. Electron validation created `/tmp/ccgui-settings-agent-nD4rUV/.commandcode/agents/electron-proof-agent.md`, confirmed Hooks bottom scroll and stacked rows, and saved screenshots `/tmp/ccgui-settings-hooks-electron.png` and `/tmp/ccgui-settings-agents-electron.png`. Browser screenshot capture timed out in the Browser backend, so Browser proof is DOM/interaction metrics plus Electron screenshots.

Status update 2026-06-06: Follow-up Settings size-awareness package constrained the native shell height chain so Settings scroll height is based on the visible Electron window instead of an oversized parent, widened Settings cards to use the available right pane, added a compact Hooks overview grid, and made compact Settings row groups multi-column where space allows. This is renderer/CSS presentation only and does not change renderer IPC, file access, config writes, hook behavior, agent behavior, session lifecycle, or Command Code runtime semantics. Validation receipts: `npm run typecheck`, `npm run build`, `npm run smoke:browser`, and constrained Electron viewport `1058x768` with `.settings-page` top `0`, bottom `768`, `clientHeight 768`, `scrollHeight 1150`, `overviewColumns 3`, `compactColumns 2`, bottom reached, and screenshots `/tmp/ccgui-settings-hooks-size-aware-electron-v3.png` and `/tmp/ccgui-settings-hooks-size-aware-electron-bottom.png`.

Status update 2026-06-06: Session terminal visibility package fixed the renderer-side xterm reset path where `TerminalPane` resubscribed and cleared its buffer when parent session readiness updates changed callback identity. Terminal exit handling is now ref-backed, active panes fit and refresh after visible layout settlement, and the session workspace/terminal card height chain is explicitly constrained for resized Electron windows. This is renderer presentation/lifecycle hygiene only; it does not add renderer IPC, broaden transport, change PTY/session lifecycle ownership, scrape terminal output for private state, mutate files/config, or alter Command Code invocation semantics. Validation receipts: `npm run typecheck`, `npx vitest run` -> `173/173`, `npm run build`, `npm run smoke:browser`, `npm run smoke:pty`, built Browser route `http://127.0.0.1:58221/` demo session preserving `browser terminal visibility proof` after sidebar collapse with no console warnings/errors, and built Electron `1306x768` demo session preserving `terminal visibility proof` after sidebar resize with screenshot `/tmp/ccgui-session-terminal-visible-electron.png`. Real CLI conversation output was not manually exercised in this package; PTY health was smoke-tested and the renderer fix is transport-agnostic.

Status update 2026-06-06: Settings UX task-flow package added `docs/reports/SETTINGS_UX_TASK_FLOW_AUDIT.md`, extended Settings registry metadata with task-flow role/group labels, added a shared Settings page header, and reworked Profile, Integrations, and Advanced into task-oriented surfaces. Profile now leads with a setup checklist, recommended next actions, and grouped project/integration/reference links; Integrations groups tasks into Set up, Create or edit, and Inspect; Advanced is relabeled as Diagnostics and no longer duplicates MCP, Hooks, Agents, Memory, Taste, or Design setup routes. Dense Settings pages now receive purpose/status/scope headers through the shared route frame. This package did not add runtime behavior, renderer IPC, filesystem reach, Command Code config mutation, transport calls, or session lifecycle changes. Validation receipts: `npm run typecheck`, `npx vitest run` -> `174/174`, `npm run build`, `npm run smoke:browser`, built Browser route `http://127.0.0.1:60721/` DOM receipts for Profile, Integrations, Diagnostics, Hooks, Agents, MCP, Data, and Sessions with no console warnings/errors, and built Electron `1058x768` screenshots `/tmp/ccgui-settings-ux-profile-electron.png`, `/tmp/ccgui-settings-ux-integrations-electron.png`, `/tmp/ccgui-settings-ux-diagnostics-electron.png`, and `/tmp/ccgui-settings-ux-hooks-electron.png`. Browser screenshot capture timed out, so Browser proof is DOM/interaction metrics plus Electron screenshots.
